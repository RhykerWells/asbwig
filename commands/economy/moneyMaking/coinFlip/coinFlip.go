...
var Command = &dcommand.AsbwigCommand{
	...
	Run: func(data *dcommand.Data) {
		...
		cooldown, err := models.EconomyCooldowns(models.EconomyCooldownWhere.GuildID.EQ(data.GuildID), models.EconomyCooldownWhere.UserID.EQ(data.Author.ID), models.EconomyCooldownWhere.Type.EQ("coinflip")).One(context.Background(), common.PQ)
		if err == nil {
		...
		if rand.Int63n(2) == 1 {
			cash = cash + bet
			embed.Description = fmt.Sprintf("You flipped %s and won %s%s", betSide, guild.EconomySymbol, humanize.Comma(bet))
