...
var Command = &dcommand.AsbwigCommand{
	...
	Run: func(data *dcommand.Data) {
		...
		cooldown, err := models.EconomyCooldowns(models.EconomyCooldownWhere.GuildID.EQ(data.GuildID), models.EconomyCooldownWhere.UserID.EQ(data.Author.ID), models.EconomyCooldownWhere.Type.EQ("snakeeyes")).One(context.Background(), common.PQ)
		if err == nil {
		...
		embed.Description = fmt.Sprintf("You rolled %d & %d, and you %s %s%s", d1, d2, condition, guild.EconomySymbol, humanize.Comma(bet))
